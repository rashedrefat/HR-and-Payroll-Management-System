  ...
  return (
    ...
    >
      <img
        src={icon}
        className={`${
          !startIcon ? "order-3" : "order-1"
        } ${iconWidth} ${iconClassName} ${disabled && "opacity-60"}`}
      />
      {text && (
        <span
          className={`text-sm font-public-sans	 font-medium leading-snug ${
            hideTextOnResponsive ? "hidden sm:inline" : "inline"
          } ${!startIcon ? "order-1" : "order-3"} ${
            disabled && "text-[#98A1B2]"
          }`}
        >
          {text}
        </span>
